...
export function Navigation() {
  ...
  const navLinks = [
    ...
    { label: 'How It Works', path: '/', scrollTo: 'how-it-works' },
    { label: 'Testimonials', path: '/', scrollTo: 'testimonials' },
    { label: 'Contact', path: '/', scrollTo: 'footer' },
  ];
        ...
        <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden lg:flex items-center gap-8">
              {navLinks.map((link) => (
                <button
                  key={link.label}
                  onClick={() => handleNavClick(link.path, link.scrollTo)}
                  className={`text-gray-700 hover:text-[#007AB6] transition-colors ${
                    isActive(link.path) ? 'text-[#007AB6]' : ''
                  }`}
                >
                  {link.label}
                </button>
